    ...
    <>
      ...
      <Box pt="64px" mb="32px">
        ...
        <Flex flexWrap="wrap" maxWidth={[0, "400px", "600px", "1200px"]} margin="0 auto">
          {data.map((el) => (
            <Commodity key={el.id} name={el.name} oldPrize={el.originalPrice} newPrize={el.specialPrice} url={el.url} images={el.images} onClick={() => {
              setVisible(true)
      ...
      <Modal visible={visible} onClose={() => { setVisible(false) }} fullHeight={true} width={1000}>
        <Flex>
          ...
          <Box width={1 / 2}>
            <Text fontSize="46px" fontWeight="700" mb="24px">{modalData.name}</Text>
            <Text fontSize="22px" mb="48px" lineHeight="1.6em" whiteSpace="pre-line">
              {modalData.description}
            </Text>
            <Text fontSize="46px" fontWeight="700" mb="24px">訂購資訊</Text>
            <form onSubmit={handleSubmit(onSubmit)}>
              <Input label="您的姓名*" error={errors.name} id="name" {...register("name", { required: {value: true, message: "請填寫欄位" }, maxLength: { value: 20, message: "限20字以內"} })} />
              <Input label="聯絡電話*" type="tel" error={errors.tel} id="tel" {...register("tel", { required: {value: true, message: "請填寫欄位" }, maxLength: { value: 15, message: "限15字以內"} })} />
              <Input label="您的E-mail*" error={errors.email} id="email" {...register("email", { required: {value: true, message: "請填寫欄位" }, pattern: {value: /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z]+$/, message: "格式錯誤"} })} />
              <Input label="寄送地址*" error={errors.address} id="address" {...register("address", { required: {value: true, message: "請填寫欄位" }, maxLength: { value: 40, message: "限40字以內"} })} />
              ...
              {Object.keys(errors).length > 0 && <Text color="#B70000" fontSize="22px" mt="16px" textAlign="center">尚有資訊未完成，請檢查您所填寫的資料</Text>}
            </form>
          </Box>
        </Flex>
      </Modal>
      <Modal visible={successModalVisible} onClose={() => { setSuccessModalVisible(false) }} fullHeight={false} width={600}>
        <Text fontSize="46px" fontWeight="700" textAlign="center" mb="24px">已收到您的訂購資料</Text>
        <Text fontSize="22px" textAlign="center" lineHeight="1.6em" mb="24px">請您留意您的E-mail電子信箱及手機簡訊，<br/>我們將儘快寄出您所訂購的商品！ <br/>謝謝！</Text>
        <Button onClick={() => { setSuccessModalVisible(false) }}>返回商品頁</Button>
      </Modal>
      <Footer />
    </>
  )
